...
public class PM {
    ...
    public static <T> void putObject(String key, final T object) {
        putObject(key, object, false);
    }
    ...
    public static <T> void putList(String key, final T list) {
        putObject(key, list);
    }
    ...
    public static <T> List<T> getList(String key, Class<T> clazz) {
        checkInitialization();
        List<Object> objectList = new Gson().fromJson((String) get(key, ""), new ListTypeToken<>(Object.class));
        if(objectList == null){
            ...
            Object entry = objectList.get(0);
            if(!entry.getClass().getName().equals(clazz.getName())){
                clear(key);
                return new ArrayList<>();
            }
        }
